...
namespace Bonsai.ONI
{
    ...
    [Description("Calculates 3D position using a light house sensor data frame.")]
    public class LightHouseFrameToPosition
    {
        // Template pattern
        ...
        public LightHouseFrameToPosition()
        {
            P = new Point3d(0, 0, 0);
            Q = new Point3d(1, 0, 0);

            var fill1 = new double[template.Count() / 4];
            var fill2 = new bool[template.Count()];
            pulse_times = new Queue<double>(fill1);
            pulse_widths = new Queue<double>(fill1);
            pulse_parse = new Queue<bool>(fill2);
        }

        bool Process(LightHouseDataFrame source)
        {
            // Push pulse time into buffer and pop oldest
            pulse_times.Dequeue();
            pulse_times.Enqueue(source.Time);

            // Push pulse width into buffer and pop oldest
            pulse_widths.Dequeue();
            pulse_widths.Enqueue(source.PulseWidth);
        ...
        public IObservable<Position3D> Process(IObservable<LightHouseDataFrame> source)
        {
            return source.Where(input => Process(input)).Select(input => new Position3D(position_time, position));
        }
